...
- **Tabla `sys.config`:** Parámetros como expiración de JWT y razón social. Ver valores
  esperados en `docs/modelo_datos_v1.md`.
...
## 7. Especificación Visual y de Interfaz (UI/UX)

- **Fondo:** Usa `/login_background.jpg` ocupando el 100% del viewport (`min-h-screen`).
- **Tarjeta Central:** Centrada con glassmorphism (`backdrop-blur-md`), bordes suavemente redondeados (`rounded-2xl`) y sombra elevada.
- **Header del Formulario:**
  - Ícono superior decorativo `PawPrint` en contenedor circular (`bg-emerald-500/10 text-emerald-500`).
  - Título principal: Razón social obtenida dinámicamente desde `GET /api/config/public`[cite: 9].
  - Subtítulo: "¡Bienvenido!".
- **Campos del Formulario:**
  - **Usuario:** Input de texto plano (no email) con ícono `User`[cite: 7, 9].
  - **Contraseña:** Input de clave con ícono `Lock` y botón integrado `Eye`/`EyeOff` para alternar visibilidad (ver/ocultar).
  - **Botón Submit:** "Iniciar Sesión" (estilo primario Emerald).
  - **Recuperación:** Enlace "¿Olvidaste tu contraseña?" debajo del botón, apuntando a `/olvide-contrasena`.
- **Exclusiones Estrictas:**
  - PROHIBIDO agregar autenticación social (Google, Facebook, etc.).
  - PROHIBIDO agregar checkbox de "Mantener sesión iniciada" / "Recordarme".
  - PROHIBIDO agregar enlace de "Registrarse" / "Crear cuenta".